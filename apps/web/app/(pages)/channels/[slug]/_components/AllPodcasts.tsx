...
import { Play, Clock, Headphones, CalendarDays, Search, X, List } from 'lucide-react';
  ...
  return (
    <section className="w-full max-w-7xl mx-auto px-4 md:px-8 py-12">
      ...
      >
        ...
        <span className="text-white/30 font-sans text-sm">{podcasts.length} episodes</span>
      </motion.div>
        ...
        <AnimatePresence mode="popLayout">
          {filtered.length === 0 ? (
            ...
            >
              ...
              <p className="text-white/40 font-sans text-base">No episodes match your search.</p>
            </motion.div>
          ) : (
            filtered.map((podcast, i) => (
              <motion.div
                ...
                layout
              >
                <PodcastRow podcast={podcast} index={i} isLast={i === filtered.length - 1} />
              </motion.div>
            ))
          )}
        </AnimatePresence>
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            {podcast.tags.map(tag => (
              <span
                key={tag}
                className="text-[10px] font-sans font-bold text-white/40 border border-white/10 px-2 py-0.5 rounded-full group-hover:border-cyan-500/20 group-hover:text-cyan-400/60 transition-colors"
              >
                {tag}
              </span>
            ))}
        ...
        <div className="flex sm:flex-col items-center sm:items-end gap-3 sm:gap-1.5 flex-shrink-0 text-xs font-sans text-white/35 sm:min-w-[100px]">
          <div className="flex items-center gap-1.5">
            ...
            <span className="font-semibold text-white/60">{podcast.duration}</span>
          </div>
          <div className="flex items-center gap-1.5">
