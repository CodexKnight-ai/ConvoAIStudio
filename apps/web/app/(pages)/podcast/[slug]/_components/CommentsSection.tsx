...
import { MessageSquare, Heart, CornerDownRight, ChevronDown, Send } from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between border-b border-white/5 pb-4">
        <h3 className="text-lg font-secondary font-bold text-white flex items-center gap-2">
          ...
          <span>Comments ({comments.length + comments.reduce((acc, c) => acc + c.replies.length, 0)})</span>
        </h3>
        ...
        <div className="flex items-center gap-1.5 text-sm">
          ...
          >
            <option value="top" className="bg-zinc-950 text-white">Top Comments</option>
      ...
      <form onSubmit={handleAddComment} className="flex gap-3">
        ...
        <div className="flex-1 relative">
          <textarea
            ...
            className="w-full min-h-[80px] p-3 rounded-2xl bg-white/5 border border-white/5 hover:border-white/10 focus:border-cyan-500/50 text-white font-sans text-sm outline-none resize-none transition-all placeholder:text-white/30"
          />
          <button
            type="submit"
        ...
        {sortedComments.map((comment) => (
          <div key={comment.id} className="space-y-4">
            ...
            <div className="flex gap-3 group">
              ...
              <div className="flex-1 space-y-1.5">
                <div className="flex items-baseline gap-2">
                  ...
                  <span className="text-white/40 font-sans text-xs">{comment.timestamp}</span>
                </div>

                <p className="text-white/80 font-sans text-sm leading-relaxed">{comment.content}</p>
              ...
              <div className="flex gap-3 ml-12 pl-4 border-l border-white/5">
                <div className="flex-1 relative">
                  <input
                    ...
                    className="w-full px-4 py-2 rounded-xl bg-white/5 border border-white/5 focus:border-cyan-500/50 text-white font-sans text-xs outline-none placeholder:text-white/30"
                  />
                  <button
            ...
            {comment.replies.map((reply: Comment) => (
              <div key={reply.id} className="flex gap-3 ml-12 pl-4 border-l border-white/5">
                ...
                <div className="flex-1 space-y-1">
                  <div className="flex items-baseline gap-2">
                    <span className="font-sans font-bold text-white text-xs">{reply.author}</span>
                    <span className="text-white/40 font-sans text-[10px]">{reply.timestamp}</span>
                  </div>

                  <p className="text-white/80 font-sans text-xs leading-relaxed">{reply.content}</p>
