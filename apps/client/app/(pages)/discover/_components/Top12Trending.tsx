...
export function Top12Trending() {
  return (
    <div className="w-full max-w-7xl mx-auto px-4 py-16">
      <h2 className="text-2xl font-secondary font-bold text-white mb-8 tracking-wide">
        More Trending Podcasts
      </h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {TOP_12.map((podcast, i) => (
          <div
            key={podcast.id}
            className="group relative liquid-glass-strong hover:bg-white/[0.03] rounded-[2rem] p-4 transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_0_20px_rgba(8,145,178,0.2)] flex items-center gap-4"
          >
            {/* Thumbnail */}
            ...
            {/* Rank Number */}
            <div className="absolute -top-3 -left-3 w-8 h-8 rounded-full liquid-glass-strong flex items-center justify-center text-xs font-bold font-sans text-white/60 shadow-md">
              {i + 4}
            </div>
          </div>
        ))}
