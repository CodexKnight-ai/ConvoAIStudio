...
export function PodcastPreviewCard({
  ...
  return (
    <Link href={`/podcast/${podcast.slug}`} className="block group">
      <motion.div
        className="relative bg-[#0d0d0d] rounded-2xl p-4 transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_0_20px_rgba(8,145,178,0.2)] flex flex-col h-full"
      ...
      >
        ...
        <div className="relative aspect-video rounded-xl overflow-hidden mb-3 bg-zinc-900 group-hover:scale-[1.02] transition-transform duration-300">
          ...
          <div className="absolute bottom-2 right-2 px-2 py-0.5 rounded bg-black/70 text-white text-xs font-medium backdrop-blur-sm z-10">
            {podcast.duration}
          </div>
          ...
          {isPlayingVideo && (
            <button
              onClick={toggleMute}
              className="absolute top-2 right-2 w-7 h-7 rounded-full bg-black/60 hover:bg-black/80 flex items-center justify-center text-white transition-colors z-20"
            >
              {isMuted ? (
