...
import { ArrowUpRight, User, History, Clock, Tv, LogOut, ChevronDown } from "lucide-react";
  ...
  return (
    <nav className="fixed top-4 left-0 right-0 z-50 flex items-center justify-between px-6 lg:px-12 max-w-7xl mx-auto">
      ...
      <div className="hidden md:flex items-center gap-1 bg-black/60 backdrop-blur-md rounded-full px-2 py-1 shadow-lg border border-white/10">
        <Link href="/" className="px-4 py-2 text-xs font-semibold text-white/70 font-sans hover:text-white transition rounded-full hover:bg-white/5 whitespace-nowrap">Home</Link>
        <Link href="/discover" className="px-4 py-2 text-xs font-semibold text-white/70 font-sans hover:text-white transition rounded-full hover:bg-white/5 whitespace-nowrap">Discover</Link>
        <Link href="/feed" className="px-4 py-2 text-xs font-semibold text-white/70 font-sans hover:text-white transition rounded-full hover:bg-white/5 whitespace-nowrap">Feed</Link>
        <Link href="/channels" className="px-4 py-2 text-xs font-semibold text-white/70 font-sans hover:text-white transition rounded-full hover:bg-white/5 whitespace-nowrap">Channels</Link>
      </div>
        ...
        {isAuthenticated ? (
          ...
          <div className="relative" ref={dropdownRef}>
            <button 
              ...
              className="flex items-center gap-2 bg-white/5 hover:bg-white/10 border border-white/10 rounded-full px-3 py-1.5 transition cursor-pointer text-white"
            >
            ...
            {dropdownOpen && (
              <div className="absolute right-0 mt-2.5 w-56 rounded-2xl bg-[#0e1320] border border-white/10 p-1.5 shadow-2xl z-50 animate-fade-in">
                <div className="px-3 py-2.5 border-b border-white/5 mb-1.5">
                  <p className="text-xs text-white/40 font-sans">Signed in as</p>
                  <p className="text-sm font-semibold text-white truncate font-sans">{user?.email}</p>
                </div>
          ...
          <div className="flex items-center gap-1.5">
            <Link href="/login" className="px-4 py-2 text-xs font-semibold text-white/80 hover:text-white transition rounded-full hover:bg-white/5 whitespace-nowrap">
              Login
            </Link>
            <Link href="/sign-up" className="px-4 py-2 text-xs font-semibold bg-white/10 hover:bg-white/15 text-white transition rounded-full border border-white/10 whitespace-nowrap">
              Sign Up
            </Link>
          </div>
        )}
