...
import { History, Clock, Tv, Plus, AlertCircle, Check, Shield, Mail, User as UserIcon, Loader2, Globe, Lock, EyeOff } from 'lucide-react';
...
function ProfileContent() {
  ...
  const [activeTab, setActiveTab] = useState<'history' | 'watch-later' | 'channels'>('channels');

  useEffect(() => {
    if (tabParam === 'history' || tabParam === 'watch-later' || tabParam === 'channels') {
  ...
  const [channelVisibility, setChannelVisibility] = useState<'PUBLIC' | 'PRIVATE' | 'UNLISTED'>('PUBLIC');
  ...
  const fetchMyChannels = async () => {
    ...
    try {
      ...
      setChannelsError(err.response?.data?.message || "Failed to retrieve your channels. Please make sure the server is running.");
    } finally {
  ...
  const handleCreateChannel = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSubmitError(err.response?.data?.message || "Failed to create channel. Please try again.");
    } finally {
        ...
        <div className="min-h-[300px]">
          <AnimatePresence mode="wait">
            ...
            {activeTab === 'channels' && (
              ...
              >
                <div className="flex items-center justify-between">
                  <div>
                    <h2 className="text-xl font-bold font-secondary">Channels You Own</h2>
                    <p className="text-sm text-white/40">Manage your broadcast channels and publish podcasts.</p>
                  </div>
                  {channels.length > 0 && (
                    <button
                ...
                {loadingChannels ? (
                  <div className="flex flex-col items-center justify-center py-16 gap-3">
                    <Loader2 className="w-8 h-8 text-cyan-400 animate-spin" />
                    <span className="text-white/40 text-sm">Loading your channels...</span>
                  </div>
                ) : channelsError ? (
                  <div className="liquid-glass border border-red-500/20 rounded-2xl p-6 flex items-start gap-3 text-red-400">
                    <AlertCircle className="w-5 h-5 shrink-0 mt-0.5" />
                    <div>
                      <h4 className="font-semibold text-sm">Error Loading Channels</h4>
                      <p className="text-xs text-white/60 mt-1">{channelsError}</p>
                      <button 
                  ...
                  <div className="liquid-glass rounded-3xl p-12 text-center border border-white/5 space-y-6 max-w-xl mx-auto mt-6">
                    ...
                    <div className="space-y-2">
                      ...
                      <p className="text-white/40 text-sm max-w-sm mx-auto font-sans leading-relaxed">
                        Create a channel to start publishing podcasts, growing your audience, and hosting discussions.
                      </p>
                    </div>
                    <button
                      onClick={() => setShowCreateModal(true)}
                      className="inline-flex items-center gap-2 px-6 py-3 rounded-full bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-black font-bold text-sm tracking-wide shadow-lg shadow-cyan-500/10 hover:shadow-cyan-500/20 transition cursor-pointer"
                    >
                      <Plus className="w-4 h-4 stroke-[3]" />
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {channels.map((channel) => (
                      <div
                        key={channel.id}
                        className="liquid-glass border border-white/10 rounded-2xl p-6 flex flex-col justify-between hover:border-cyan-500/30 transition-all duration-300 group"
                      >
                        <div className="space-y-3">
                          <div className="flex items-start justify-between">
                            <h3 className="text-lg font-bold font-secondary text-white group-hover:text-cyan-300 transition-colors">
                              {channel.name}
                            </h3>
                            <span className="text-xs font-semibold px-2 py-0.5 rounded-md bg-white/5 text-white/60">
                              /{channel.slug}
                            </span>
                          </div>
                        ...
                        <div className="flex items-center justify-between border-t border-white/5 mt-6 pt-4 text-xs text-white/40">
                          <span>
            ...
            {activeTab === 'history' && (
              ...
              >
                <div>
                  <h2 className="text-xl font-bold font-secondary">Listening History</h2>
                  <p className="text-sm text-white/40">Podcasts you have listened to recently.</p>
                </div>
                
                {/* Beautiful Mock History display */}
                <div className="grid grid-cols-1 gap-4">
                  {[
                    { id: '1', title: 'GPT-5 Architecture & Release Speculations', channel: 'AI Frontier', duration: '45m', date: '2 hours ago' },
                    { id: '2', title: 'Neuralink Human Trials: The First 100 Days', channel: 'BioTech Daily', duration: '1h 12m', date: 'Yesterday' }
                  ].map((item) => (
                    <div key={item.id} className="liquid-glass border border-white/5 hover:border-white/10 transition-all duration-200 rounded-xl p-4 flex items-center justify-between gap-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-white/40 mt-0.5">{item.channel} • {item.duration}</p>
                        </div>
                      </div>
            ...
            {activeTab === 'watch-later' && (
              ...
              >
                <div>
                  <h2 className="text-xl font-bold font-secondary">Saved for Later</h2>
                  <p className="text-sm text-white/40">Keep track of podcasts you want to watch or listen to later.</p>
                </div>

                {/* Beautiful Mock Watch Later display */}
                <div className="grid grid-cols-1 gap-4">
                  {[
                    { id: '1', title: 'Quantum Computing meets Artificial Intelligence', channel: 'Quantum Future', duration: '32m', date: 'Added 3 days ago' }
                  ].map((item) => (
                    <div key={item.id} className="liquid-glass border border-white/5 hover:border-white/10 transition-all duration-200 rounded-xl p-4 flex items-center justify-between gap-4">
                      <div className="flex items-center gap-3">
            ...
            <div className="p-6 md:p-8 space-y-6">
              ...
              {submitSuccess ? (
                <div className="flex flex-col items-center justify-center py-10 gap-3 text-cyan-400">
                  ...
                  <h4 className="font-bold font-secondary">Channel Created Successfully!</h4>
                  <p className="text-xs text-white/40 font-sans">Refreshing your channel list...</p>
                </div>
              ) : (
                <form onSubmit={handleCreateChannel} className="space-y-4">
                  <div className="space-y-1">
                    <label className="text-xs font-semibold text-white/60 font-sans">Channel Name</label>
                    <input
                      type="text"
                  ...
                  <div className="space-y-1">
                    <label className="text-xs font-semibold text-white/60 font-sans">Description</label>
                    <textarea
                  ...
                  <div className="space-y-1">
                    <label className="text-xs font-semibold text-white/60 font-sans">Visibility</label>
                    <div className="grid grid-cols-3 gap-2">
                      {[
                        { id: 'PUBLIC', label: 'Public', icon: Globe, desc: 'Anyone can view' },
                        { id: 'PRIVATE', label: 'Private', icon: Lock, desc: 'Only you can view' },
                        { id: 'UNLISTED', label: 'Unlisted', icon: EyeOff, desc: 'Viewable via link' },
                      ].map((v) => {
                        const Icon = v.icon;
                        ...
                        return (
                          ...
                          >
                            ...
                            <span className="text-xs font-bold font-sans">{v.label}</span>
                          </button>
                        );
                      })}
                    </div>
